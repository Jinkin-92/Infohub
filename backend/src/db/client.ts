...
function initSQLiteTables() {
  ...
  // 添加 is_public 和 public_source_id 列到 sources 表（SQLite 不支持 IF NOT EXISTS）
  const columnCheck = sqliteClient.prepare(
    "SELECT COUNT(*) as cnt FROM pragma_table_info('sources') WHERE name='is_public'"
  );
  const isPublicExists = (columnCheck.get() as { cnt: number }).cnt > 0;

  if (!isPublicExists) {
    sqliteClient.exec(`ALTER TABLE sources ADD COLUMN is_public INTEGER DEFAULT 0`);
  }

  const publicSourceIdCheck = sqliteClient.prepare(
    "SELECT COUNT(*) as cnt FROM pragma_table_info('sources') WHERE name='public_source_id'"
  );
  const publicSourceIdExists = (publicSourceIdCheck.get() as { cnt: number }).cnt > 0;

  if (!publicSourceIdExists) {
    sqliteClient.exec(`ALTER TABLE sources ADD COLUMN public_source_id INTEGER REFERENCES public_sources(id)`);
  }
